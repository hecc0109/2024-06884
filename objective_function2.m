function[ z,cv] =objective_function2( x  )%输入
...
%% ε约束方法  基于目标与约束分离的方法  该方法使用参数ε来放松约束。ε逐渐减小，当个体的约束违反程度小于ε时，视为可行解
...
penalty_factor1=1e3;
 global epsilon;
    if cv <= epsilon
        ...
        Etotal = Etotal + penalty_factor1 * (cv - epsilon);
         dis_ukm=dis_ukm+penalty_factor1 * (cv - epsilon) ;
    end 
