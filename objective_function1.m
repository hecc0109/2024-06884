function[ z,cv] =objective_function1( x  )
...
for i = 1:num_cells
  ...
  for j=1:num_cells1
    ukm{i}{j}= sum(Ui_m{1, i}{1, j}  ); %无人机m服务k区域的满意度
    sum_ukm=sum_ukm+ukm{i}{j};%总满意度
  end
end
%% 不满意度
dis_ukm=1*user_num-sum_ukm;%不满意度 完全满意度-满意度
...
for i = 1:num_cells1
  ...
  for j=1:num_cells1
    E1{i}{j}= sum(Eicom_m{1, i}{1, j}  ); %无人机m服务k区域的计算能耗
    Etotal_com=Etotal_com+E1{i}{j};%总的计算能耗
  end
end
%悬停能耗
...
E=cell2mat(E);%每个无人机能耗

for m=1:M
    g(:,m)=E(:,m);%g<=60每个无人机的最大能耗不超过60
    ...
    elseif g(:,m) <0
        ...
        violationDegree(:,m) = 0;  % 如果满足约束，则违反程度为0
    end
end
...
%% 罚函数法   通过在目标函数中加入惩罚项，将约束优化问题转化为无约束优化问题。
...
global penalty_factor;
    if cv <= 0
        ...
        Etotal = Etotal + penalty_factor * cv ;
        dis_ukm=dis_ukm+penalty_factor * cv ;
    end
