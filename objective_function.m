function[ z,cv] =objective_function( x  )
...
%  end
 ...
 if dis_ukm > 0.6  
      ...
      violationDegree(:,4) = (dis_ukm  - 0.6)*140;
    else
        violationDegree(:,4) = 0;  
 end
...
%% 定义优化目标 满意度最大化 能耗最小化  在满意度前加负号，使两个目标函数同时最小化 
 z1=dis_ukm;%最小化不满意度 最小化时延
% z2=Etotal_com;%最小化能耗  时延小不满意度小 能耗大 
 z2=Etotal;%%最小化能耗  时延小不满意度小 导致能耗大 对立冲突 
z = [z1 z2]';%输出
end